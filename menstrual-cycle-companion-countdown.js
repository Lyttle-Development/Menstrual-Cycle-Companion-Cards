...
class MenstruationCountdownTimer extends HTMLElement {
    ...
    symptomGrid.querySelectorAll('input[type="checkbox"][data-symptom-key]').forEach(checkbox => {
      checkbox.addEventListener('change', async () => {
        ...
        try {
          ...
          await this.callService("menstrual_cycle_companion", "add_symptom", serviceData);
  ...
  async _doLogFirstPeriod() {
    try {
      ...
      await this.callService("menstrual_cycle_companion", "add_symptom", {
        ...serviceBase,
        date: today,
      ...
      await this.callService("menstrual_cycle_companion", "log_first_period", {
        ...serviceBase,
        date: today,
      });
  ...
  async logProductUsage(action = "used") {
    try {
      ...
      await this.callService("menstrual_cycle_companion", "log_product_usage", {
        entity_id: this.config?.entity,
        profile: this.config?.profile,
        entry_id: this.config?.entry_id,
  ...
  _saveTimerState() {
    try {
      ...
      this.callService("menstrual_cycle_companion", "save_timer_state", serviceData)
  ...
  _loadTimerState() {
    try {
      ...
      const timerStateObj = this._hass.states[`menstrual_cycle_companion_timer.${profile}`];
