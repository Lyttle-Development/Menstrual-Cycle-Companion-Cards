  ...
  async _handleModalSave() {
    ...
    if (allowPeriodToggle && wantsPeriod !== hasPeriod) {
      try {
        ...
        console.error('menstrual-calendar-card: failed to toggle period', err);
      }
    }
    ...
    if (!allowPeriodToggle && autoConfirmDays.length > 0) {
      try {
        ...
        // eslint-disable-next-line no-console
        console.error('menstrual-calendar-card: failed to auto-confirm period day', err);
      }
    }

    if (Object.keys(symptomData).length > 0) {
      try {
        await this._hass.callService('menstrual_cycle_companion', 'add_symptom', {
          date: iso,
        ...
        // eslint-disable-next-line no-console
        console.error('menstrual-calendar-card: failed to save symptoms', err);
      }
    }
  ...
  _render() {
      ...
      <ha-card>
        <div class="wrap" id="root" style="position:relative">
          ...
          <div class="legend">
            ...
            ${model.nfpAnalysis && model.nfpAnalysis.ovulation_detected && model.nfpAnalysis.confidence_level !== 'low' ? `<span class="legend-item nfp-method-badge"><img src="/menstrual_cycle_companion/assets/state/nfp.svg" alt="NFP Method" class="method-icon" /><span>NFP</span></span>` : (model.sensorOvulation ? `<span class="legend-item nfp-method-badge"><img src="/menstrual_cycle_companion/assets/state/hybrid.svg" alt="Standard Method" class="method-icon" /><span>Standard</span></span>` : '')}
          </div>
        </div>
      </ha-card>
    ...
    root?.addEventListener('click', (ev) => {
      ...
      if (iso) {
        ...
        this._modalIso = iso;
        this._render();
        this.dispatchEvent(new CustomEvent('menstrual-calendar-day-click', {
