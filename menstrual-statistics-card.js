...
// (formerly menstrual-product-stats-shared.js)
// ---------------------------------------------------------------------------
  ...
  _renderNfpTab(attrs) {
    ...
    if (!this._hasNfpData(attrs)) {
      return `<div class="section">
        <div class="section-header"><span class="section-icon"><img src="/menstrual_cycle_companion/assets/state/nfp.svg" alt="NFP" class="section-icon-img" /></span><span>${esc(t('nfp_title'))}</span></div>
    ...
    const methodLabel = (nfp.ovulation_detected && confidence !== 'low')
      ? `<img src="/menstrual_cycle_companion/assets/state/nfp.svg" alt="NFP Method" class="method-icon" />${esc(t('nfp_method_nfp'))}`
      : `<img src="/menstrual_cycle_companion/assets/state/hybrid.svg" alt="Standard Method" class="method-icon" />${esc(t('nfp_method_standard'))}`;
    ...
    return `
      <div class="section">
        <div class="section-header">
          <span class="section-icon"><img src="/menstrual_cycle_companion/assets/state/nfp.svg" alt="NFP" class="section-icon-img" /></span>
          <span>${esc(t('nfp_title'))}</span>
          <span class="nfp-confidence-badge ${confidenceClass}">${esc(confidenceLabel)}</span>
        </div>
        <div class="nfp-info-box">
          <div class="nfp-info-row">
            <span class="nfp-info-icon"><img src="/menstrual_cycle_companion/assets/state/nfp.svg" alt="NFP" class="nfp-info-icon-img" /></span>
  ...
  _attachListeners() {
    ...
    root.querySelectorAll('button[data-action="add-underwear-shopping"]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        await this._hass.callService('menstrual_cycle_companion', 'manage_household_inventory', {
    ...
    if (exportBtn) {
      exportBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          await this._hass.callService('menstrual_cycle_companion', 'export_doctor_report', serviceData);
