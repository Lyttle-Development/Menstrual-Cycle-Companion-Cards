...
class MenstruationGaugeCard extends HTMLElement {
  ...
  _renderGauge(model, palette) {
    ...
    if (this._validSeries60(model.series60)) {
      ...
      return `
        <svg class="gauge" viewBox="0 0 420 420" width="100%" height="100%" role="img" aria-label="Menstrual Cycle Companion 60 days">
    ...
    return `
      <svg class="gauge" viewBox="0 0 420 420" width="100%" height="100%" role="img" aria-label="Menstrual Cycle Companion">
  ...
  _calendarGrid(model, locale) {
    ...
    for (let i = 0; i < totalCells; i++) {
      ...
      const inPendingRange = pendingStart && pendingEnd && iso >= pendingMin && iso <= pendingMax;
      const rangeStart = active && !model.confirmedSet.has(previousIso);
      const rangeEnd = active && !model.confirmedSet.has(nextIso);
      items.push(`<button class="day ${active ? 'active' : ''} ${today ? 'today' : ''} ${hasSymptoms ? 'has-symptoms' : ''} ${rangeStart ? 'range-start' : ''} ${rangeEnd ? 'range-end' : ''} ${inPendingRange ? 'pending-range' : ''}" type="button" data-iso="${iso}">${day}</button>`);
    }
    return items.join('');
  }
  ...
  _confirmedRangeForDate(iso, model = this._buildModel()) {
    ...
    const adjacent = (dateIso, offset) => {
      const value = this._parseISO(dateIso);
      if (!value) return '';
      value.setDate(value.getDate() + offset);
      return this._isoFromDate(value);
    };
    const range = [iso];
    let cursor = adjacent(iso, -1);
    while (model.confirmedSet.has(cursor)) {
      range.unshift(cursor);
  ...
  async _setCycleRange(startIso, endIso) {
    ...
    const attempts = [
      base,
      { start_date: startIso, end_date: endIso, ...(profile ? { profile } : {}) },
  ...
  async _handleModalSave() {
    ...
    if (allowPeriodToggle && wantsPeriod !== hasPeriod) {
      try {
        ...
        console.error('menstrual-cycle-card: failed to toggle period', err);
      }
    }
    ...
    if (!allowPeriodToggle && autoConfirmDays.length > 0) {
      try {
        ...
        // eslint-disable-next-line no-console
        console.error('menstrual-cycle-card: failed to auto-confirm period day', err);
      }
    }
    ...
    if (Object.keys(symptomData).length > 0) {
      try {
        ...
        try {
          await this._hass.callService('menstrual_cycle_companion', 'add_symptom', payload);
        } catch (_) {
          await this._hass.callService('menstrual_cycle_companion', 'add_symptom', payload);
        }
      } catch (err) {
        // eslint-disable-next-line no-console
        console.error('menstrual-cycle-card: failed to save symptoms', err);
      }
    }
  ...
  async _doLogFirstPeriod() {
    try {
      ...
      try {
        await this._hass.callService('menstrual_cycle_companion', 'add_symptom', addSymptomPayload);
      } catch (_) {
        await this._hass.callService('menstrual_cycle_companion', 'add_symptom', addSymptomPayload);
      }
      ...
      try {
        await this._hass.callService('menstrual_cycle_companion', 'log_first_period', logFirstPeriodPayload);
      } catch (_) {
        await this._hass.callService('menstrual_cycle_companion', 'log_first_period', logFirstPeriodPayload);
      }
      ...
      console.error('menstrual-cycle-card: error logging first period', error);
      this._removeFirstPeriodModal();
    }
  }
  ...
  _attachHandlers() {
    ...
    const grid = this.shadowRoot;
    grid?.addEventListener('contextmenu', (ev) => {
      const dayBtn = ev.target?.closest?.('.day[data-iso]');
      if (!dayBtn || this._config?.calendar_edit_enabled === false) return;
      ev.preventDefault();
      this._deleteCalendarSelection(dayBtn.getAttribute('data-iso'));
    });
    grid?.addEventListener('pointerdown', (ev) => {
      ...
      this._calendarLongPressTimer = setTimeout(() => {
          ...
          if (iso) {
            ...
            if (rangeMode) {
              if (!this._rangeStart || this._rangeEnd) {
                ...
                this._toggleInFlight = true;
                this._setCycleRange(start, end)
                  .then(() => this._refreshSensorEntity(this._buildModel().entityId))
                  .then(() => {
                    this._rangeStart = '';
                    this._rangeEnd = '';
                    this._render();
                  })
                  .catch((err) => console.error('menstrual-cycle-gauge-card: failed to set cycle range', err))
                  .finally(() => { this._toggleInFlight = false; });
              }
            } else {
              this._modalIso = iso;
              this._render();
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .day.pending-range { background: color-mix(in srgb, var(--primary-color) 24%, ${palette.dayBg}); border-color: var(--primary-color); box-shadow: inset 0 -3px 0 var(--primary-color); }
          ...
          <div class="wrap">
            ...
            <div class="editor">
              ${String(this._config?.calendar_selection_mode || 'range').toLowerCase() === 'range'
                ? `<div class="range-help">${this._rangeStart && !this._rangeEnd ? `Start selected: <strong>${this._rangeStart}</strong> — click the end date.` : 'Click the first day, then the last day of the cycle.'} Right-click a saved day, or hold it on mobile, to remove its contiguous range.</div>`
                : ''}
              <div class="toolbar">
...
class MenstruationGaugeCardEditor extends HTMLElement {
  ...
  _attachHandlers() {
    ...
    this._onEditorChange = (ev) => {
      ...
      if (target.id === 'calendar_selection_mode') return this._handleInput('calendar_selection_mode', target.value === 'toggle' ? 'toggle' : 'range');
    };
...
for (const [name, base] of [
  ['menstrual-gauge-card', MenstruationGaugeCard],
  ['menstrual-cycle-gauge-card', MenstruationGaugeCard],
  ['menstruation-gauge-card', MenstruationGaugeCard],
  ['menstrual-gauge-card-editor', MenstruationGaugeCardEditor],
  ['menstrual-cycle-gauge-card-editor', MenstruationGaugeCardEditor],
  ['menstruation-gauge-card-editor', MenstruationGaugeCardEditor],
]) {
  if (!customElements.get(name)) customElements.define(name, class extends base {});
}
